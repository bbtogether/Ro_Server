...
// defined by eric wang for ro comb orchistrated command
var  ROCOMB_MESSAGE =         0x0A; 
// Rocomb command bytes
var ROCOMB_CONFIG  =             0x10;
var ROCOMB_Z_RETRACT =           0x20;
var ROCOMB_Z_EXTEND =            0x30;
var ROCOMB_EN_STEPPER_Z =        0x40;
var ROCOMB_EN_STEPPER_LR =       0x50;
var ROCOMB_EN_LCD =              0x60;
var ROCOMB_EN_MPOWER =           0x70;
var ROCOMB_EN_SPOWER =           0x80;
var ROCOMB_TEST =                0x90; //flush pin 13 LED for 3 times

var ROCOMB_MODE_MASK =           0xF0;
...
SYSEX_RESPONSE[SERIAL_MESSAGE] = function(board) {
  ...
  if (command === (SERIAL_REPLY)) {
    for (var i = 3, len = board.currentBuffer.length; i < len - 1; i += 2) {
      //reply.push(((board.currentBuffer[i + 1]& 0x7F) << 7) | (board.currentBuffer[i])&0x7F);  //changed by eric wang for test
 ...
 *   bytesToRead {number} The number of bytes to read on each iteration of the main loop
...
Board.prototype.serialConfig = function(options) {
  ...
  // TODO - if info about the Serial capabilities can be added to the capability query, a
  // reasonable default value can be added and this error can be removed. Until then it is
  // not possible to know if a board supports only HW serial or only SW serial or both so
  // there is really no way to add a default portId value.
...
Board.prototype.serialRead = function(portId, callback) {
  var data = [
    START_SYSEX,
    SERIAL_MESSAGE,
    SERIAL_READ | portId,
    this.SERIAL_MODES.READ_CONTINUOUS,
    END_SYSEX
  ];
  this.transport.write(new Buffer(data));
...
/**
 * Handles the message from a rocomb completing action
 * @param {Board} board
 */

SYSEX_RESPONSE[ROCOMB_MESSAGE] = function(board) {
  var response = board.currentBuffer[2];
  board.emit("rocomb-action-done",response);
};

/**
 * For Rocomb Test only. by Eric wang to trigger the test command
 * @param {number} count The number of times to flash the LED on pin 13.
 */

Board.prototype.rocombTest = function(count, callback) {
  ...
  this.on("rocomb-action-done", callback);
};
